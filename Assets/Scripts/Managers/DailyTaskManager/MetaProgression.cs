...
public class MetaProgression : MonoBehaviour
{
    ...
    public void SaveData() // Changed to public so TutorialManager can call it - Archie | [25/09/25].
    {
    ...
    public void StartActualGame() // Changed to StartActualGame so it's more descriptive - Archie | [25/09/25].
    {
    ...
    void CheckForUnlocks()
    {
        ...
        if (
            data.totalWavesCompleted >= wavesForWeaponUnlock
            && !data.unlockedWeapons.Contains("Advanced Rifle")
        )
        {
        ...
        if (
            data.totalWavesCompleted >= wavesForWeaponUnlock * 2
            && !data.unlockedWeapons.Contains("Plasma Cannon")
        )
        {
        ...
        if (
            data.totalBossesKilled >= bossesForAbilityUnlock
            && !data.unlockedAbilities.Contains("Shield")
        )
        {
        ...
        if (
            data.totalBossesKilled >= bossesForAbilityUnlock * 2
            && !data.unlockedAbilities.Contains("Time Slow")
        )
        {
        ...
        if (
            data.totalEnemiesKilled >= killsForCosmeticUnlock
            && !data.unlockedCosmetics.Contains("Golden Armor")
        )
        {
        ...
        if (
            data.totalEnemiesKilled >= killsForCosmeticUnlock * 2
            && !data.unlockedCosmetics.Contains("Particle Trail")
        )
        {
