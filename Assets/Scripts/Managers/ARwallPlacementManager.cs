...
public class ARWallPlacementManager : MonoBehaviour
{
    ...
    [SerializeField]
    private bool horizontalPlanesOnly = true;

    [Header("Placeables")]
    [SerializeField]
    private List<WallType> wallTypes = new();
    ...
    [SerializeField]
    private bool aimWithScreenCenter = true;
    ...
    {
        if (!arCamera)
            arCamera = Camera.main ? Camera.main : GetComponentInChildren<Camera>();
        if (!raycastManager)
            raycastManager =
                GetComponent<ARRaycastManager>() ?? FindObjectOfType<ARRaycastManager>();
        if (!planeManager)
    ...
    void Update()
    {
        if (!previewEnabled || !raycastManager || !arCamera || wallTypes.Count == 0)
            return;
    ...
    {
        rotationSteps = (rotationSteps + 3) & 3;
    }
    ...
    {
        if (!previewEnabled || !currentCanPlace || !currentPlane)
            return;
        ...
        if (!CanPlaceAt(currentPlane, originCell, type))
            return;
        ...
        switch (type.kind)
        {
            case WallKind.SolidWall:
                ...
                {
                    var obs =
                        go.GetComponent<NavMeshObstacle>() ?? go.AddComponent<NavMeshObstacle>();
                    obs.carving = true;
    ...
    private void UpdatePreview(Vector2 screenPos)
    {
        ...
        Vector3 forward = Vector3
            .ProjectOnPlane(arCamera.transform.forward, currentPlane.transform.up)
            .normalized;
        if (forward.sqrMagnitude < 1e-4f)
            forward = currentPlane.transform.forward;
    ...
    {
        var type = CurrentType();
        if (ghost != null && type != null && ghost.name.Contains(type.name))
            return;
        RebuildGhost();
    }
...
public class DamageZone : MonoBehaviour
{
    ...
    void OnTriggerEnter(Collider other)
    {
        ...
        if (e)
            e.ApplyDamageMultiplier(damageMultiplier);
    }
